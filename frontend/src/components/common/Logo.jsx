...
export function KBLogoIcon({ className = 'h-6 w-6' }) {
  return (
...
export function Logo({ size = 'md', className, subtitle }) {
  ...
  return (
    <div className={cn('flex items-center select-none', containerSizes[size], className)}>
      <KBLogoIcon className={iconSizes[size]} />
      {size !== 'sm' && (
        <div className="flex flex-col leading-none">
          <span className="text-[15px] font-extrabold tracking-tight text-slate-900 font-display uppercase">
            Knowledge <span className="text-slate-500 font-semibold lowercase text-xs tracking-normal font-sans ml-0.5">base</span>
          </span>
          {subtitle && (
