      ...
      {open && (
        <div className="fixed inset-0 z-[80] flex items-start justify-center p-4 pt-[12vh]">
          ...
          <motion.div
            ...
            className="relative w-full max-w-xl overflow-hidden rounded-2xl border border-slate-200/90 bg-white/95 backdrop-blur-2xl shadow-2xl"
          >
            {/* Top cyan-to-purple gradient line */}
            <div className="h-1 w-full bg-gradient-to-r from-cyan-500 via-teal-400 to-purple-500" />

            <div className="flex items-center gap-3 border-b border-slate-100 px-4.5">
              <Search size={17} className="shrink-0 text-cyan-600" />
              <input
                ref={inputRef}
                value={query}
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          ...
                          isActive ? 'bg-cyan-50/80 text-cyan-800 font-semibold' : 'bg-transparent text-slate-700',
                        )}
                      >
                        {item.kind === 'action' ? (
                          <>
                            <item.icon size={16} className={cn('shrink-0', isActive ? 'text-cyan-600' : 'text-slate-400')} />
                            <span className="flex-1 truncate text-[13px]">{item.label}</span>
                            {item.hint && <span className="text-[11px] font-medium text-slate-400">{item.hint}</span>}
                          </>
                        ) : (
                          <>
                            <FileText size={16} className={cn('shrink-0', isActive ? 'text-cyan-600' : 'text-slate-400')} />
            ...
            <div className="flex items-center gap-4 border-t border-slate-100 bg-slate-50/60 px-4.5 py-2.5 text-[11px] font-medium text-slate-400">
